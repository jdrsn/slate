...
If you are sending raw HTTP requests, the API key has to be sent in the query param `k`  or in a custom HTTP header also called `k`.

> <a href='https://qi.do/r/test/prod?k=mY4p1k3y' target='_blank'>qi.do/r/test/prod?k=mY4p1k3y </a>
...
If your app has authentication enabled, every object automatically includes the user id of whom created that object.
...
In order to make any object private, you can set the property `_p` as follows:
...
```json
{
  ...
  "channel": "kids" // custom prop
}
```

The `endpoint` and `keys` properties are generated by the user's browser when permissions are granted.
You are free to add any other properties to the subscription object.
These properties can then be used to filter custom audiences. 
...
<br/>
The only required properties are title and body.
A complete list of all available properties can be found <a href="https://developer.mozilla.org/en-US/docs/Web/API/notification#properties" target="_blank">here</a>.
...
<br/>
The properties `to`, `sub` and `msg` are required, while `att` is optional.
